...
-----------------------------  Test Mutex  ---------------------------------
  ...
  function TestMutex ()
      ...
      thArr[0].Fork (LockTester, 0)

      thArr[1].Init ("LockTester-B")
      thArr[1].Fork (LockTester, 0)

      thArr[2].Init ("LockTester-C")
      thArr[2].Fork (LockTester, 0)

      thArr[3].Init ("LockTester-D")
      thArr[3].Fork (LockTester, 0)

      thArr[4].Init ("LockTester-E")
      thArr[4].Fork (LockTester, 0)

      thArr[5].Init ("LockTester-F")
      thArr[5].Fork (LockTester, 0)

      thArr[6].Init ("LockTester-G")
      thArr[6].Fork (LockTester, 0)
...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    -- The number of buffer space left is indicated by SEmpty
    SEmpty: Semaphore = new Semaphore
    -- The number of filled positions in the buffer is indicated by SFull
    SFull: Semaphore = new Semaphore
    -- lock for the producer so that no more than one producer can produce at a time
    ProducerLock: Mutex = new Mutex
    -- lock for the consumer so that no more than one consumer can consume at a time
    ConsumerLock: Mutex = new Mutex
    -- lock for print as it is globally accessed
    PrintLock: Mutex = new Mutex

  function ProducerConsumer ()
      -- Initialize producer, comsumer and print locks
      ProducerLock.Init()
      ConsumerLock.Init()
      PrintLock.Init()
      -- Initialize SFull as 0 because initially there are 0 filled positions in our buffer
      SFull.Init(0)
      -- Initialize SEmpty to BUFFER_SIZE beacuse all the positions in our buffer are empty in the begining
      SEmpty.Init(BUFFER_SIZE)
      print ("     ")
  ...
  function Producer (myId: int)
      ...
      for i = 1 to 5
        ...
        -- call SEmpty Down to decrement the value of count (Initialized to BUFFER_SIZE) 
        -- before acquiring the lock to check if the buffer is full
        SEmpty.Down()
        
        -- acquire lock so that one prodcer can produce
        ProducerLock.Lock()
        
        -- Add c to the buffer
        buffer [bufferNextIn] = c
        bufferNextIn = (bufferNextIn + 1) % BUFFER_SIZE
        -- acquiring lock for print before bufferSize as it is used in PrintBufferfunction
        PrintLock.Lock()
        ...
        -- releasing the lock for print
        PrintLock.Unlock()
        
        -- Perform synchronization...
        -- Increment the value of count in SFull to indicate one position is filled in the buffer
        SFull.Up()

        -- release the ProducerLock
        ProducerLock.Unlock()     
      endFor
    endFunction
  ...
  function Consumer (myId: int)
      ...
      while true
        -- Perform synchroniztion...
        -- Decrement the value of count in SFull to indicate one positon is emptied/consumed
        -- before acquiring the lock to check if the buffer is empty or there is an underflow
        SFull.Down()
        
        -- acquire lock so that one consumer consumes at a time
        ConsumerLock.Lock()
        
        -- Remove next character from the buffer
        c = buffer [bufferNextOut]
        -- acquiring lock for print as bufferNextOut and bufferNextOut are used in PrintBufferfunction
        PrintLock.Lock()
        ...
        -- Perform synchronization...
        -- Increment the value of count in SEmpty to indicate one position is emptied in the buffer
        SEmpty.Up()

        -- release the ConsumerLock
        ConsumerLock.Unlock()
      endWhile
    endFunction
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  class ForkMonitor
    ...
    fields
      ...
      -- Lock for the monitor entry menthod
      m1: Mutex
      -- declare condition variables for each philosophers in order to delay when hungry or signal
      c1: array [5] of Condition
    methods
      Init ()
      PickupForks (p: int)
      PutDownForks (p: int)
      -- declare test method to check statusof neighbours
      test1 (i: int)
      PrintAllStatus ()
  endClass
  ...
  behavior ForkMonitor
    ...
    method Init ()
      ...
      c1 = new array [5] of Condition {5 of new Condition}
      for i = 0 to 4
        c1[i].Init ()
      endFor
    ...
    method PickupForks (p: int)
      ...
      mon.PrintAllStatus()
      -- call test function which checks if left and right neighbors are eating when you are hungry
      mon.test1(p)

      -- wait if one of the neighbors are eating
      if(status[p] != EATING) 
        c1[p].Wait(&m1)
      endIf
      -- release monitor mutex lock
      m1.Unlock()
      endMethod
    ...
    method PutDownForks (p: int)
      ...
      status[p] = THINKING
      mon.PrintAllStatus()
      -- if right neighbor is hungry and
      -- both of rights neighbors are not eating,
      -- set right state to eating and wake it up by signaling
      mon.test1((p+1) % 5)
      mon.test1((p-1) % 5)
      m1.Unlock()
      endMethod
    ...
    method test1(i: int)
      if status[i] == HUNGRY && status[(i+1) % 5] != EATING && status[(i-1) % 5] != EATING
        ...
        status[i] = EATING
        mon.PrintAllStatus ()
        -- wake up hungry philosophers during PutDownForks
        c1[i].Signal(&m1)
      endIf
    endMethod
