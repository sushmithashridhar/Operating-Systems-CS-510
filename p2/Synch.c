  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Lock ()
          var
            oldIntStat: int
            -- Check if lock is held by the current thread
          if Threadinexecution == currentThread
            FatalError ("Current thread holds the lock")
          endIf
            -- disable interrupts 
          oldIntStat = SetInterruptsTo (DISABLED)
            -- set current thread to hold the lock if lock is free
          if !Threadinexecution
            Threadinexecution = currentThread
          else
            -- Current thread is put to sleep as lock is not available
            waitingThreads.AddToEnd (currentThread)
            currentThread.Sleep ()
          endIf
          -- enable interrupts
      ...
      method Unlock ()
          var
            ...
            -- Check if lock is held by the current thread
          if Threadinexecution != currentThread
            FatalError ("urrent thread does not hold the lock")
          endIf
          -- diable interrupts
          oldIntStat = SetInterruptsTo (DISABLED)
          ...
          t = waitingThreads.Remove ()
          if t
            -- assign ready status to next thread in list
            t.status = READY
            readyList.AddToEnd (t)
            Threadinexecution = t
          else
            Threadinexecution = null
          endIf
          -- disable interrupts
          oldIntStat = SetInterruptsTo (oldIntStat)
        endMethod
      ...
      -- This function checks if lock is held by current thread
            if(Threadinexecution == currentThread)
              return true
            else
              return false
