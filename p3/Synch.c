...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Lock ()
          var interrupt : int
          interrupt = SetInterruptsTo (DISABLED)
          if hold == null
            hold = currentThread
          else
            waitingThreads.AddToEnd(currentThread)
            currentThread.Sleep()
          endIf
          interrupt = SetInterruptsTo (interrupt)
        endMethod
      ...
      method Unlock ()
          ...
          interrupt = SetInterruptsTo(DISABLED)
           if hold != currentThread
            FatalError(" Current thread is not holding the lock")
          else 
            threadptr = waitingThreads.Remove()
            if threadptr
              threadptr.status = READY
              readyList.AddToEnd(threadptr)
              hold = threadptr                      
            else
              hold = null
            endIf
