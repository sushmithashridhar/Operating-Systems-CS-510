...
function InitFirstProcess()
  -- We are creating a User level process, inorder to achieve it, we need some 
  -- initialization which needs to be done for user thread. 
  -- Hence in InitFirstProcess we are getting a new thread object by invoking 
  -- the function GetANewThread(). Once we obtain the thread we initialize the 
  -- thread and call fork to get the thread running, which executes StartUserProcess.

  var
      t: ptr to Thread
      -- Obtain a new thread object
      t = threadManager.GetANewThread()
      t.Init("UserProgram")
      -- Execute the function StartUserProcess
...
-- StartUserProcess is used in starting up the user-level process.
-- We allocate a process control block and point the PCB to the thread
-- as the PCB can point to a list of threads.
  var
      addrSpace: ptr to AddrSpace
      testfile: ptr to OpenFile
      initPC: int
  ...
  -- create a logical address space and load the executable to the program counter
  -- if it does not fetch return the entry point
  initPC = testfile.LoadExecutable(&PCB.addrSpace)
  if(initPC == -1)
    print("Not able to get PC")
  endIf

  -- close the excecutable file
  fileManager.Close(testfile)
  ...
  -- disable interrupts to avoid race Condition while setting page table registers
  -- and making thread as user level thread
  oldIntStat = SetInterruptsTo (DISABLED)

  -- initializing page tables registers for the logical address space
  PCB.addrSpace.SetToThisPageTable ()

  currentThread.isUserThread = true

  -- change the mode from system mode to user mode. Enable interrupts and paging
  -- jumps to user level main routine and the user thread will not return 
...
-----------------------------  PrintReadyList  ---------------------------------
  ...
  function PrintReadyList ()
    ...
    -- printing to guarantee that the readyList wont change while it is
...
-----------------------------  ThreadStartMain  ---------------------------------
  ...
  function ThreadStartMain ()
    ...
    -- invoke the threads "main" function, with interrupts enabled.  If the "main"
...
behavior HCondition
    ...
    -- means that in the time between a Signal() operation and the awakening
    -- and execution of the corrsponding waiting thread, no other threads run in between
    -- Signal() and wake up of corresponding waiting thread
    --
    -- This class provides the following methods:
    --    Wait(mutex)
    --         This method assumes the mutex has alreasy been locked.
    ...
    --         If there are any threads waiting on this condition, this
    --         method will wake up the oldest and schedule it to run.
    --         When the signal is received, this method
    --         hand overs the mutex to next waiting thread and the waiting
    --         thread will acquire the lock before leaving Signal()
    --         Any other thread arriving, needs to wait and can run only if the 
    --         currentThread leaves the monitor mutex or goes to wait.
    --
    --    Broadcast(mutex)
    --         This method is like Signal() except that it wakes up all
    --         threads waiting on this condition, not just the next one.
    ...
    --         Each condition must be initialized.
      ...
      method Wait (mutex: ptr to Mutex)
          ...
          -- check if the Wait() is called when the monitor lock is not held
          if ! mutex.IsHeldByCurrentThread ()
            FatalError ("Attempt to wait on condition when mutex is not held")
          endIf
      ...
      method Signal (mutex: ptr to Mutex)
          var
            ...
            -- check if the signal() is called when the monitor lock is not held
          if ! mutex.IsHeldByCurrentThread ()
            FatalError ("Attempt to signal a condition when mutex is not held")
          endIf
          ...
          nl()
          nl()
          print(".....................Handing over the mutex lock.....................")
          nl()
          nl()
          nl()
          ...
          if t
            ...
            -- the signaller gains access immediately
           

            -- status of the next waiting thread is put to READY
            t.status = READY
            ...
            mutex.heldBy = t 
            -- The thread which calls the signal will get the access
            mutex.Lock()
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        -- 
          ...
          aThreadBecameFree.Init()
            ...
            threadTable[0].Init("Thread-1")
            threadTable[1].Init("Thread-2")
            threadTable[2].Init("Thread-3")
            threadTable[3].Init("Thread-4")
            threadTable[4].Init("Thread-5")
            threadTable[5].Init("Thread-6")
            threadTable[6].Init("Thread-7")
            threadTable[7].Init("Thread-8")
            threadTable[8].Init("Thread-9")
            threadTable[9].Init("Thread-10")

          -- setting status of each thread to UNUSED and once
          -- the status is put to UNUSED , add it to freeList.
          -- Because the threads in freeList has to have status as UNUSED, hence
          -- we change the status first and then add to freeList
      ...
      method GetANewThread () returns ptr to Thread
        ...
        -- until one is available.
          ...
          -- the while loop will wait until a thread becomes available
          while freeList.IsEmpty()
            aThreadBecameFree.Wait(&threadManagerLock)
          endWhile
          
          -- when the freeList is not empty , remove it from the freeList
          -- and change its status from UNUSED to JUST_CREATED
          t = freeList.Remove()
      ...
      method FreeThread (th: ptr to Thread)
        ...
        -- 
          ...
          -- It adds a thread to freeList and then signals
          --if anyone wants , who are in the waiting Condition
          freeList.AddToEnd(th)

          -- when put to freeList , need to change the status to UNUSED
          th.status = UNUSED
  ...
  behavior ProcessManager
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        ...
        --
          ...
          -- the while loop will wait until a process becomes available
          while freeList.IsEmpty()
            aProcessBecameFree.Wait(&processManagerLock)
          endWhile
          
          -- when the freeList is not empty , remove a process from the freeList
          -- and change its status from FREE to ACTIVE, also assign a process ID
            t = freeList.Remove()
            t.status = ACTIVE
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        --
          ...
          -- It adds a process to freeList and then 
          freeList.AddToEnd(p)

          -- when put to freeList , need to change the status to FREE
          p.status = FREE

          --signals if anyone wants , who are in the waiting Condition
  ...
  behavior FrameManager
      ...
      method Init ()
        ...
        var i: int
          ...
          LeaderThread = new Condition
          LeaderThread.Init()
          RestOfThreads = new Condition
          RestOfThreads.Init ()
      ...
      method GetAFrame () returns int
        ...
        --
          ...
          while numberFreeFrames < 1
            RestOfThreads.Wait (&frameManagerLock)
          endWhile
      ...
      --  This method needs to make a note of which frames have
      -- been allocated. It does this by storing the address 
      -- of each frame it allocates to the AddrSpace which is the page taable

          var f, i, frameAddr: int
          frameManagerLock.Lock ()

          -- Increment the number of threads waiting for the frame
          numberofwaitingthreads = numberofwaitingthreads + 1

          -- if the number of threads already waiting is more than 1
          -- add the thread to waiting list
          if( numberofwaitingthreads > 1)
            RestOfThreads.Wait(&frameManagerLock)
          endIf

          -- if the number of frames available is less than the 
          -- number of frames needed by the thread then put the 
          -- first thread to wait.
          while numberFreeFrames < numFramesNeeded
            LeaderThread.Wait (&frameManagerLock)
          endWhile

          -- find which frames are free and set them to 1
          for i=0 to numFramesNeeded-1
            f = framesInUse.FindZeroAndSet ()
            
            -- find the address of the free frame
            frameAddr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (f * PAGE_SIZE)

            -- store address of the frame in the aPageTable
            aPageTable.SetFrameAddr(i,frameAddr)
          endFor

          -- Decrement the number of frames that has been used from total free frames
          numberFreeFrames = numberFreeFrames - numFramesNeeded
          aPageTable.numberOfPages = numFramesNeeded


          -- signal the next waiting thread and decrement the count of waiting threads
          numberofwaitingthreads = numberofwaitingthreads-1
          RestOfThreads.Signal(&frameManagerLock)
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
          ...
          var numFramesReturned, bitNumber, i, frameAddr : int

          frameManagerLock.Lock ()

          numFramesReturned = aPageTable.numberOfPages

          --check the page table and see which frames are set
          -- free them by clearing the bit, by setting it to 0
          ...
          -- Increment the total free frames available
          numberFreeFrames = numberFreeFrames + numFramesReturned

          -- signal the leader thread who is waiting for the frames
          LeaderThread.Signal(&frameManagerLock)
...
-----------------------------  AddrSpace  ---------------------------------
  ...
  behavior AddrSpace
      ...
      method SetDirty (entry: int)
        --
        -- Set the selected page table entrys "dirty" bit to 1.
        --
      ...
      method SetReferenced (entry: int)
        --
        -- Set the selected page table entrys "referenced" bit to 1.
        --
      ...
      method SetWritable (entry: int)
        --
        -- Set the selected page table entrys "writable" bit to 1.
        --
      ...
      method SetValid (entry: int)
        --
        -- Set the selected page table entrs "valid" bit to 1.
        --
      ...
      method ClearDirty (entry: int)
        --
        -- Clear the selected page table entrs "dirty" bit.
        --
          pageTable[entry] = pageTable[entry] & ! 0x00000008
        endMethod
      ...
      method ClearReferenced (entry: int)
        --
        -- Clear the selected page table entrys "referenced" bit.
        --
          pageTable[entry] = pageTable[entry] & ! 0x00000004
        endMethod
      ...
      method ClearWritable (entry: int)
        --
        -- Clear the selected page table entrys "writable" bit.
        --
          pageTable[entry] = pageTable[entry] & ! 0x00000002
        endMethod
      ...
      method ClearValid (entry: int)
        --
        -- Clear the selected page table entrys "valid" bit.
        --
          pageTable[entry] = pageTable[entry] & ! 0x00000001
        endMethod
      ...
      method CopyBytesFromVirtual (kernelAddr, virtAddr, numBytes: int)
                    returns int
        --
        -- This method copies data from a users virtual address space
      ...
      method CopyBytesToVirtual (virtAddr, kernelAddr, numBytes: int)
                    returns int
        --
        -- This method copies data from the kernels address space to
      ...
      method GetStringFromVirtual (kernelAddr: String, virtAddr, maxSize: int) returns int
        ...
        -- a pointer to an "array of char" in the kernels code.  This method
...
-----------------------------  Handle_Sys_Exit  ---------------------------------
  ...
  -- function prints exit status which tells thread of execution will be made to stop running
...
-----------------------------  Handle_Sys_Shutdown  ---------------------------------
  ...
  -- prints shutdown statement which indicates that now all the process will stop running
...
-----------------------------  Handle_Sys_Yield  ---------------------------------
  ...
  -- prints yield status , indicating to allow execution of thread that is ready to run
...
-----------------------------  Handle_Sys_Fork  ---------------------------------
  ...
  -- prints that a fork is invoked    
  -- and return a unique number
...
-----------------------------  Handle_Sys_Join  ---------------------------------
  ...
  -- prints which process ID needs to terminate until which caller function waits
  -- and returns a unique number
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  -- In the exec function it runs the executable file in the context of 
  -- already existing process. Hence we dont need to allocate a Process control 
  -- block once again. To load another executable we need one more virtual address space.
  -- We should return the frames for old adress space once we are done Initializing the 
  -- new address space as it would need to return to the process that invoked Exec.
      ...
      var 
          ...
          -- filename pointer address is converted from virtual address to physical address  
          i = currentThread.myProcess.addrSpace.GetStringFromVirtual(&strBuffer, filename asInteger,MAX_STRING_SIZE)

          if(i<0)
            -- print("Did not find a match to physical adress for virtual adress to perform copy")
            return -1
          endIf

          -- perform file open
          testfile = fileManager.Open(&strBuffer)
          ...
          -- create a logical address space and load the executable to the program counter
          -- if it does not fetch return the entry point
          initPC = testfile.LoadExecutable(&newAddrSpace)
          if(initPC == -1)
            -- print("Not able to get PC")
          return -1
          endIf
          ...
          -- return all the frames allocated for the previous address space 
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  -- prints the virtual address of the file 
  -- and returns a unique number
    var 
        i: int
        strBuffer: array [MAX_STRING_SIZE] of char
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  -- print the virtual address of the file which has been opened
  -- and returns a unique number
      var
          strBuffer: array [MAX_STRING_SIZE] of char
          i: int
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  -- prints the virtual address of the file which is being read, the size of file in Bytes
  -- and returns a unique number
...
-----------------------------  Handle_Sys_Write  ---------------------------------
  ...
  -- prints the virtual address of the file on which write is invoked
  -- and returns a unique number
...
-----------------------------  Handle_Sys_Seek  ---------------------------------
  ...
  -- prints the file and the current postion the seek is on
...
-----------------------------  Handle_Sys_Close  ---------------------------------
  ...
  -- prints the file which is being closed
  ...
  behavior FileManager
      ...
      method Init ()
        ...
        var i: int
          ...
          -- "Stub System" directory page.  Well just keep this around
      ...
      --
          ...
          -- If we didnt find a matching name, return null
          if numFiles <= 0
            return null
          endIf
